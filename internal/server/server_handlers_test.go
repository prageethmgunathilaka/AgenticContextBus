...
func (s *inMemoryAgentStore) Create(ctx context.Context, a *models.Agent) error {
    if s.m == nil { s.m = map[string]*models.Agent{} }
    if _, ok := s.m[a.ID]; ok {
        return nil
    }
    cp := *a
...
func (s *inMemoryAgentStore) Get(ctx context.Context, id string) (*models.Agent, error) {
    if a, ok := s.m[id]; ok { cp := *a; return &cp, nil }
    return nil, fmt.Errorf("agent not found")
}
func (s *inMemoryAgentStore) Update(ctx context.Context, a *models.Agent) error {
    if _, ok := s.m[a.ID]; !ok { return fmt.Errorf("agent not found") }
    cp := *a
    s.m[a.ID] = &cp
    return nil
}
...
func (s *inMemoryAgentStore) List(ctx context.Context, f *storage.AgentFilters) ([]*models.Agent, error) {
    res := []*models.Agent{}
    for _, a := range s.m { cp := *a; res = append(res, &cp) }
    return res, nil
}
func (s *inMemoryAgentStore) UpdateLastSeen(ctx context.Context, id string) error {
    if a, ok := s.m[id]; ok { a.LastSeen = time.Now(); a.Status = models.AgentStatusOnline; return nil }
...
func TestRefreshTokenEndpoint(t *testing.T) {
    ...
    if err != nil { t.Fatalf("gen refresh: %v", err) }
...
func TestAgentHandlers_CRUD(t *testing.T) {
    ...
    req.Header.Set("Content-Type", "application/json")
    httpSrv.router.ServeHTTP(w, req)
    if w.Code != http.StatusCreated { t.Fatalf("register expected 201, got %d", w.Code) }
    ...
    req.Header.Set("Authorization", hdr)
    httpSrv.router.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("heartbeat expected 200, got %d", w.Code) }
